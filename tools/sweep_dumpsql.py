...
from probe_dumpsql import python_sql, zc_sql, load, _CHECK_PROJECTIONS, _typed_projections, _CONFORMANCE_QUERY, _TYPED_NODES_QUERY

EXTRA = {"strview": ("collections",), "str": ("collections",), "with_alias": ("collections",)}

names = sorted(os.path.splitext(os.path.basename(p))[0] for p in glob.glob("examples/*.z"))
for unit in names:
    try:
    ...
    for t, q in _typed_projections(len(units)).items():
        if py.execute(q, units).fetchall() != zp.execute(q, units).fetchall():
            bad.append(t)
    if py.execute(_CONFORMANCE_QUERY).fetchall() != zp.execute(_CONFORMANCE_QUERY).fetchall():
        bad.append("conformance")
    if py.execute(_TYPED_NODES_QUERY, (unit,)).fetchall() != zp.execute(_TYPED_NODES_QUERY, (unit,)).fetchall():

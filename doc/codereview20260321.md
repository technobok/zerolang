...
- [x] Track emitter output lines → source AST node IDs for debuggability — `CEmitter.source_map` maps each C output line to its originating AST `nodeid` via `TrackedList` on `struct_defs`/`func_defs`/`data_defs`

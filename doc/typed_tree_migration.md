...
- `Node.type` (~90 writers across typecheck) and `Node.const_value`
  (~25 writers). Big refactor; same side-table pattern applies but
  every `node.type` / `node.const_value` read in typecheck has to
  route through the table. The two TypedExpression-derived classes
  already carry `ztype` / `const_value`; the side-tables move what
  the typecheck currently writes to parsed `Node` into a per-nodeid
  store, then `_build_typed_*` reads from there.
- `Expression.call_kind` — wrapper-only; the typed tree intentionally
  has no `TypedExpression` wrapper class per the original design.
  Options: introduce a wrapper class, move `call_kind` to a
  TypeChecker side-table the emitter accesses (would need exposing
  it on TypedProgram), or compute `call_kind` from the inner
  subtype's typed mirror at each emitter read site. Currently 4
  emitter read sites + several typecheck reads; deferred until a
  decision is made.
...
> for the original design. Pick up Step 6.9.b (strip `Node.type`).

## Session handoff snapshot (after Step 6.9.a)

State of the tree at commit `88dca51`:

- **Steps 1-5 complete.** Typed tree is fully built; emitter and SQL
  dump consume it.
- **Step 6 partially complete: 9 of 10 field families stripped.**
  `Node.const_value` landed in 6.9.a (`88dca51`). The TypeChecker
  side-table pattern is proven at scale across multiple regressions
  (as-items, unit-level if, generic-type-match) — see the Step-6.9.a
  notes above for the three flow fixups that came out of running
  the full test suite.
- **`make test` 1962 passing** (full suite incl. emitter+gcc).

Remaining for Step 6:

1. **Step 6.9.b — `Node.type`.** ~90 writers across `ztypecheck.py`,
   ~50 readers in `ztypecheck.py` proper, plus 174 read sites in
   `zemitterc.py` (most already routed through `_node_ztype` /
   `_path_ztype` helpers with parsed-fallback) and 18 in
   `zasthash.py`. Side-table approach is the same as 6.9.a; the
   complication scaled up:

   - **Typed-mirror late-stamping recurrence.** 6.9.a hit three
     fixups where a typed mirror was built before the field's value
     was settled (subject of `_check_generic_type_match`,
     unit-level if, and as-items propagation). For `.type` the
     recurrence is wider: `_check_call_inner` mutates
     `call.callable.type` after `_check_path` already built the
     callable's typed mirror — handled in 4d via a sync
     `callable_typed.ztype = cast(ZType, call.callable.type)` at
     the end of `_build_typed_call`. Several other late-stamping
     sites (`path.parent.type = enclosing` at 3537, generic param
     resolution at 5516-5523, parent_atom.type at 7218) probably
     need similar rebuilds.
   - **Typeref-vs-value duality.** `Path` serves both as a typeref
     (parameter type, return type, field type) and as a
     value-yielding expression (call callable, BinOp lhs). The
     typeref path is set by `_resolve_typeref` directly without
     going through `_check_path`. The 6.9.a as-items fix
     (`_build_typed_atomid` / `_build_typed_dotted_path` inline
     after `_resolve_typeref`) generalises — every typeref site
     needs a typed mirror built inline once `.type` is gone from
     the parsed side. Inventory the call sites of
     `_resolve_typeref` and the synthetic `path.type = X`
     assignments.
   - **`audit_type_annotations` (line ~10588).** Walks the parsed
     program looking for `node.type is None` on Path nodes. Either
     migrate to walk the typed program (every Path-shaped
     TypedExpression is typed by construction except for parents in
     numeric-cast shortcut and the `clause.match` structural
     atoms) — or retire the function since the typed-tree
     guarantee replaces the runtime check.
   - **`zasthash.hash_function`** is called from typecheck after
     body checking, with three call sites (mono dedup). It hashes
     `ppath.type`, `func.returntype.type`, and per-node `node.type`.
     Pass the TypeChecker (or just the side-table) into
     `hash_function` so it reads from the side-table; alternatively
     accept a typed-mirror lookup callback.

   Approach options (same as 6.9.a):
   - One sweep with a Python-driven script that handles writes +
     reads uniformly. Risk: regex misfires on `ZType.typetype` or
     similar non-Node `.type` accesses. Audit each replacement.
   - Per-method sub-steps mirroring 6.1-6.8 but the cross-method
     reads (`atom.type` read in `_check_dotted_path_inner` after
     written in `_check_atomid`) make per-method splitting
     awkward — the partial state needs the field present and the
     side-table populated simultaneously. Ends up doing it as one
     atomic change anyway.
   - Recommend: one big commit, ratcheted by `make test`'s
     identical-output invariant and the existing
     `_node_ztype` / `_path_ztype` emitter helpers (drop the
     parsed-fallback, same as 6.9.a did for `_node_const_value`).
...
## Step 6.9.a — what landed

- Side-table `TypeChecker._node_const_value: dict[int, ConstValue]`
  added; ~25 parsed-Node `const_value` writers in `ztypecheck.py`
  rerouted to it. Readers in `_resolve_unit_level_if`,
  `_check_binop_inner`, `_check_if_inner` (constant-false suppression)
  and `_check_case_inner` (scalar match folding) read via the
  side-table. ZType-level `const_value` writes (on `_make_type`
  shells in `_process_as_items_protocols`) left intact — those are
  on the resolved type, not on parsed nodes.
- Typed-mirror builders pull `const_value` from the side-table:
  `_build_typed_atomid`, `_typed_path_from_parsed`'s ATOMID/DOTTEDPATH
  branches, `_build_typed_atomstring`, `_build_typed_dotted_path`,
  `_build_typed_binop`, `_build_typed_call`, `_build_typed_if/case/
  for/do/with`. The structural `clause.match` TypedAtomId built
  inside `_build_typed_case` is now also registered in
  `by_parsed_id` (was previously embedded only) so emitter
  `_node_const_value(clause.match)` lookups find it — needed for
  scalar / generic-type-match arm elimination.
- `_process_as_items_protocols` builds typed mirrors inline for
  numeric ATOMID, ATOMSTRING, and the `_resolve_typeref`
  ATOMID/LABELVALUE/DOTTEDPATH propagation paths so as-constants
  carry their `const_value` on the typed tree (the emitter's
  `_const_names` collector and `_emit_dotted_path_value` consult
  the typed mirror after Step 6.9.a, no parsed fallback).
- `_resolve_unit_level_if` builds the typed `If` mirror inline
  (no `_check_if` wrapper runs for unit-level ifs) and stamps
  `const_value` on both the parsed `Expression` wrapper and the
  inner `If` so `_node_const_value`'s descend-then-lookup pattern
  finds the value via the typed `If`.
- `_check_generic_type_match` rebuilds the subject's TypedAtomId
  after stamping `const_value` on the side-table — the original was
  built during a prior `_check_atomid` traversal (or wasn't built
  at all when the subject is a generic param) and the late stamp
  needs to be reflected on the typed mirror.
- Emitter helper `_node_const_value` and SQL-dump helper of the
  same name dropped their parsed-fallback (`return node.const_value`
  → `return None`). Direct reads of parsed `node.const_value` in
  the emitter (`_emit_operation_value`, `_emit_binop_value`,
  `_emit_dotted_path_value`, `_emit_case`, `_emit_simple_case_expr`)
  rerouted to `_node_const_value(node)` or the typed-mirror's
  `const_value` field.
- `zast.Node.const_value` field declaration removed; only ZType
  carries `const_value` on the parsed side now (and that lives in
  `ztypes.py`, untouched).
- Test surface: `TestConstantFolding` helpers now look up
  `const_value` via `program.typed_program.by_parsed_id` (descending
  Expression). `TestTypedAtomIdInvariants` /
  `TestTypedDottedPathInvariants` / `TestTypedBinOpInvariants`
  drop the `typed.const_value == parsed.const_value` agreement
  assertion (the parsed field is gone) and assert the expected
  literal value where the program is determinate, or `None` for
  the dotted-numeric-cast BinOp form which doesn't propagate
  const_value to the parent AtomId.

`make check` clean, `make test-fast` 1373 passing,
`make test` 1962 passing (full suite incl. emitter+gcc).

## Step 6.9.b — what landed

`Node.type` stripped from the parser AST. The replacement is a
two-tier lookup: the typed-tree mirror remains the primary source of
truth for value-yielding expressions, with the
`TypeChecker._node_type` side-table (snapshotted onto
`TypedProgram.node_types` at end of typecheck) covering parsed
nodes whose typed mirror isn't independently registered (typeref
Path nodes inside `Function.parameters`, `Function.returntype`,
`ObjectDef.is_items`/`as_items`, plus a few synth nodes).

- `TypeChecker._node_type: dict[int, Optional[ZType]]` added.
  ~90 writers across `ztypecheck.py` (Path / Atom / DottedPath /
  AtomString / BinOp / Call / If / Case / For / Do / With / Function
  returntype / parameter resolution, generic mono retag, error
  recovery, etc.) all rerouted to the side-table via a Python-driven
  bulk transform.
- ~50 readers in `ztypecheck.py` (cross-method `path.parent.type`
  lookups in `_check_dotted_path_inner`, generic-param fixup in
  monomorphisation, `audit_type_annotations`, the
  `last_statement_type` Assignment-cast read, etc.) migrated to the
  side-table. Chain reads of the form `ppath.type.typetype` /
  `ppath.type.name` / `ppath.type.parent` rebound to a local
  (`ppath_t = self._node_type.get(ppath.nodeid)`) so the type-checker
  can narrow None-ness across uses.
- `TypedProgram.node_types: Dict[int, Optional[ZType]]` added; the
  TypeChecker copies `self._node_type` onto it in
  `_build_typed_program_units` alongside the other `resolved` /
  `mono_types` / `unit_types_by_id` snapshots. `audit_type_annotations`
  reads from this dict (was reading parsed `node.type`).
- `zasthash.hash_function` accepts a `node_types: Dict[int,
  Optional[ZType]]` parameter and threads it through every helper;
  the three call sites in `ztypecheck.py` (mono dedup) pass
  `self._node_type`. Test caller in `tests/test_asthash.py` passes
  `program.typed_program.node_types` via a small `_node_types`
  helper.
- `zemitterc.CEmitter._node_ztype` and `._path_ztype` drop the
  parsed-fallback (`return node.type` / `return path.type`) in
  favour of the TypedProgram's `node_types` side-table. ~179
  direct `<expr>.type` reads in the emitter rerouted to
  `self._node_ztype(<expr>)` via a Python-driven transform; ~30 of
  those needed inline `cast(ZType, ...)` to satisfy the type-checker
  where the same lookup is repeated across `if X and X.attr` guard
  patterns. A handful of false-positive transforms on
  `cast(zast.DottedPath, X).<attr>.type` chains (where the regex
  greedily attached `.type` to the closing `)`) were re-fixed to
  wrap the cast: `self._node_ztype(cast(zast.DottedPath, X).<attr>)`.
- `zsqldump._node_ztype` drops parsed-fallback similarly; routes
  the missing-mirror case to `typed_program.node_types`.
- `zast.Node.type` field removed. The parsed AST no longer carries
  any decoration field on the base class — only `nodeid`, `nodetype`
  (instance-tagged), `synth_origin`, and `start` remain. Step 7's
  `@dataclass(frozen=True)` on `Node` is now mechanical (only the
  `Expression.call_kind` wrapper field remains in Step 6, addressed
  in Step 6.10).
- Test surface: `TestFinding1TypeAnnotations` and the
  `TestFinding3DestructorMetadata` field-type read use a new
  `_node_ztype(program, node)` helper that consults
  `program.typed_program.node_types`. `test_typecheck.py` reads of
  `assign.type`, `inner.type`, `temp_assn.type`, `s_copy.parent.type`,
  `sline.value.type` similarly route through a module-level
  `_node_ztype` helper. `test_typed_tree.py` invariant tests rebind
  `assert typed.ztype is X.type` to `assert typed.ztype is
  _ztype_of(tc, X)` where `_ztype_of` reads
  `tc.typed_program.node_types`.

## Step 6.10 — what landed

`Expression.call_kind` stripped. The design choice from the
candidates listed in the prior session-handoff snapshot was
**option B** (TypeChecker side-table, snapshotted onto
`TypedProgram` for emitter access) — option A would have added a
TypedExpression wrapper class the design explicitly rejected, and
option C (compute on demand from inner subtype) doesn't cover the
Path-with-control-kind-resolved case where the wrapper carries the
classification but the inner Path doesn't.

- `TypeChecker._expr_call_kind: dict[int, CallKind]` added. Two
  writers in `_check_expression` (one for the Path-with-control-
  flow case, one propagating from a wrapped Call). Two readers in
  `_check_statement` (mark-unreachable on non-completing tail) and
  `_last_statement_type` (NORETURN propagation).
- `TypedProgram.expr_call_kinds: Dict[int, CallKind]` added; the
  TypeChecker snapshots `self._expr_call_kind` onto it alongside
  the Step-6.9.b `node_types` snapshot.
- `CEmitter._expr_call_kind(expr)` helper wraps the
  `typed_program.expr_call_kinds.get(expr.nodeid, UNKNOWN)`
  lookup. The 6 emitter read sites
  (`_emit_expression_stmt`'s break/continue/panic dispatch and the
  three `_emit_*_block`-shaped non-completing-tail checks) all
  route through it.
- `zast.Expression.call_kind` field declaration removed.

`make check` clean, `make test-fast` 1373 passing,
`make test` 1962 passing.

## Step 6 — done

Every `init=False` typecheck-set field on the parser AST is gone.
The parser produces nodes carrying only structural fields plus
`nodeid`, `nodetype`, `synth_origin`, and `start`. All
typecheck-derived state lives on the typed tree (`TypedNode`
hierarchy in `ztypedast.py`) and the `TypedProgram` side-tables
(`node_types`, `expr_call_kinds`, `resolved`, `mono_types`,
`unit_types_by_id`, `by_parsed_id`). Step 7
(`@dataclass(frozen=True)` on `Node`) is mechanical from here:
nothing in `ztypecheck.py` writes to a parsed node post-parse, so
the freeze is a one-line change plus whatever ratchet test guards
the boundary.

Three lessons that carried over from 6.9.a but scaled up here:
- The Python-driven bulk transform is fast but the regex needs
  defensive bounding (`(?!\s*=)` to avoid catching writes,
  `cast(...)` chain awareness). Audit `make check` output after
  the bulk transform to find misfires.
- Type-checker (`ty`) doesn't narrow `Optional[T]` across repeated
  method calls on the same expression; bind to a local or use
  `cast(T, ...)` at problem sites.

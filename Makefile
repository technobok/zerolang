...
# emitter-name-resolution:30 (typed-AST-authoritative — emitter reads stamps, not names; drive to 0)
bootstrap-lint:
	@fail=0; \
	...
	if [ $$count -gt 30 ]; then \
		echo "ERROR: emitter name-resolution calls increased ($$count > 30 baseline)"; \

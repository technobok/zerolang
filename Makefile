...
#   collections.z (10) -- the containers, which the P5 split replaces with
...
any-guard:
	...
	if [ "$$c" -gt 10 ]; then echo "any-guard FAIL: collections.z Any.generic = $$c (baseline 10)"; fail=1; fi; \
	if [ "$$fail" = "1" ]; then echo "  Lower the baseline here when a residual is legitimately removed."; exit 1; fi; \
	echo "any-guard OK: user source clean; system.z=$$s (<=7) collections.z=$$c (<=10)"

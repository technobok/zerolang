...
# emitter-name-resolution:10 (typed-AST-authoritative — emitter reads stamps, not names; drive to 0)
bootstrap-lint:
	@fail=0; \
	...
	if [ $$count -gt 10 ]; then \
		echo "ERROR: emitter name-resolution calls increased ($$count > 10 baseline)"; \

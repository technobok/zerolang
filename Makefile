...
# emitter-name-resolution:42 (typed-AST-authoritative — emitter reads stamps, not names; drive to 0)
bootstrap-lint:
	@fail=0; \
	...
	if [ $$count -gt 42 ]; then \
		echo "ERROR: emitter name-resolution calls increased ($$count > 42 baseline)"; \

...
| 2026-07-26 | 2cab67d | post-Q2 cleanup (`e09f96a`..`2cab67d`): retire the migration instrumentation (13 counters + q2SpikeVerify); delete the Decl/ZTyping/ZType state nothing reads; collapse the two `childOf` vocabularies and the two member enumerators into one each; stop allocating row lists to answer scalar questions (`declMemberCount` took FOUR, `declMoveChildToEnd` THREE per first-typed member); `recFieldLists` becomes `dataFieldNames`; drop the derived `type_children` dump table. **The allocation regression closes, the wall one does not**: allocs −7.9% and bytes churned −17%, but typecheck stays ~307ms. Under mimalloc these allocations were not what cost the time — the remaining lever is Map probing (see the 2026-07-17 cache census), not allocation. DHAT after: `String_copy` 35.9%, `checkCall` 12.2%, the whole member-enumeration family down from 896k blocks (8.4%) to 232k (2.3%). | 0.62s | 0.69s | 121MB / 113MB | 91 / 307 / 222 (total 629) | 9,921,948 | 499MB | 13.9s | 80,951 |

...
## The write-only pool is no longer zero (2026-08-12 @ ac6f307f) — OPEN LEAD

`make perf-elision` measures the allocations LLVM deletes for us: bytes written,
never read, pointer never escaping. The 2026-08-07 and 2026-08-10 rows both
report **zero**, and the target's own comment says it belongs there, because a
growing pool means emitted code that fills a buffer and frees it unread.

It is not zero. Measured across the two unrowed arcs, each at its own commit
with the same clang A/B:

| at | write-only pool |
|---|---|
| `29dd84f8` (last rowed) | 0 |
| `cca9d6b4` (native table done) | **95** |
| `d88bc68d` (runtime pass, S6a) | 107 |
| `ac6f307f` (HEAD) | **122** |

So **95 of the 122 arrived with the native-table arc**, which was never measured,
and the runtime-pass arc added 27 across its own span (12 by S6a, 15 after) —
diffuse, not one site. It is 122 allocations against 8.6 million, and the columns
that cost anything are flat, which is why it is recorded rather than chased here:
the invariant broke in an arc that is not this one, and half a hunt is worse than
a written-down number.

**What would find it:** the pool is a clang-vs-clang delta over `bin/zc.c`, so a
DHAT run on `out/zc-noelide` and a diff of allocation SITES against
`out/zc-elide` names the emitted functions directly — the two binaries differ
only in whether LLVM recognised the allocator.

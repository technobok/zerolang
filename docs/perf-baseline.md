...
- **Measured at the migration's end (2026-08-06, `5c59c47a`)**: gcc
  8,999,867 vs clang 8,246,464 -- delta 753,403, essentially unchanged from
  the ~750k measured before it. The L022 sites were dead OWNED COPIES, and
  clang was already eliding a different pool: dead malloc->memcpy->free
  chains it can prove. Removing the copies at the source shrank BOTH builds
  by the same amount. **The delta is not a progress meter for this
  migration** -- treat it as a standing estimate of what gcc's optimiser
  leaves on the table, not as a worklist.

...
Three functions in `compiler0/zemitterc.py` handle all name generation:
...
Generic types are mangled during type checking (`compiler0/ztypecheck.py:2096-2098`):
```python
...
Add a `cname` field to `ZType` in `compiler0/ztypes.py` so that every type carries its
C identifier. Populate it during type checking when the type is first resolved.

- [x] Add `cname: str = ""` field to the `ZType` dataclass in `compiler0/ztypes.py`
- [x] In type checking (`compiler0/ztypecheck.py`), after resolving each type definition
...
- [x] Add `cname TEXT` column to the `types` table in `compiler0/zsqldump.py`
...
for ex in examples/*.z; do
    name=$(basename "$ex" .z)
    uv run python compiler0/zc.py "$name" --src examples 2>/dev/null
    if [ -f "${name}.c" ]; then
...
uv run python compiler0/zc.py hello --src examples --dump-sql - | grep cname
```

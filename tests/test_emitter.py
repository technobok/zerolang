...
class TestIdenticalAndStringEquality:
    """Test identical function and string == / != C emission."""

    def test_identical_emits_pointer_compare(self):
        """identical emits C pointer == comparison."""
        csource = emit_source(
            'main: function is {\n  a: "hello"\n  x: identical lhs: a rhs: a\n}'
        )
        # should contain pointer comparison, not a function call
        assert "==" in csource

    def test_identical_compiles_and_runs(self):
        """identical returns true for same ref, false for different."""
        csource = emit_source(
            "main: function is {\n"
            '  a: "hello"\n'
            '  b: "world"\n'
            "  r1: identical lhs: a rhs: a\n"
            "  r2: identical lhs: a rhs: b\n"
            '  if r1 then print "same"\n'
            '  if r2 then print "never"\n'
            "}"
        )
        output = compile_and_run(csource)
        assert output.strip() == "same"
...
class TestBornBorrowedEmitter:
    ...
    BAG_ITERATOR = (
        ...
        "    return bagiter pos: 0 max: 3 target: b.private\n"
        "  }\n"
        "}\n"
        "bagiter: record {\n"
        "  pos: i64\n"
        ...
        "out this.borrow is {\n"
        "    return bagiter pos: 0 max: 3 target: target\n"
        "  }\n"
        "  call: function {it: this} out (optionval t: i64) is {\n"
        "    if it.pos < it.max then {\n"
        "      if it.pos == 0 then {\n"
    ...
    def test_born_borrowed_slice_runs(self):
        ...
        csource = emit_source(
            ...
            "    return cview offset: 0 source: c.private\n"
            "  }\n"
            "}\n"
            "cview: record {\n"
            "  offset: i64\n"
            ...
            "out this.borrow is {\n"
            "    return cview offset: 0 source: source\n"
            "  }\n"
            "}\n"
            "main: function is {\n"

...
class TestEmitterVariant:
    ...
    def test_variant_equality(self):
        ...
        # integer-only variant uses memcmp
...
class TestAutoGeneratedEquality:
    ...
    def test_record_eq_c_output(self):
        """Verify z_point_eq function is generated."""
        csource = emit_source(
        ...
        # integer-only record uses memcmp
        assert "memcmp" in csource
    ...
    def test_nested_record_eq_c_output(self):
        """Nested integer records both use memcmp."""
        csource = emit_source(
            "inner: record { v: i64 }\n"
        ...
        # both are memcmp-safe, outer uses memcmp directly
        assert "memcmp(&a, &b, sizeof(z_outer_t))" in csource
    ...
    def test_memcmp_eq_integer_record(self):
        ...
        assert "memcmp(&a, &b, sizeof(z_point_t))" in csource

    def test_memcmp_eq_float_uses_field_compare(self):
        """Float record does NOT use memcmp, uses field-by-field."""
        csource = emit_source(
    ...
    def test_memcmp_eq_compiles_and_runs(self):
        """memcmp-based equality compiles and works correctly."""
        csource = emit_source(
            "point: record { x: i64  y: i64 }\n"
            "main: function is {\n"
            "  a: point\n"
    ...
    def test_memcmp_eq_variant(self):
        """Integer variant uses memcmp."""
        csource = emit_source(
            "result: variant { ok: i64  err: i64 }\n"
            "main: function is {\n"
        ...
        assert "memcmp(&a, &b, sizeof(z_result_t))" in csource

...
_BUILD_SKIP = {"mathutil", "genmath", "ifctl", "forctl", "doctl", "withctl"}
...
class TestAutoGeneratedEquality:
    ...
    def test_bool_value_eq_uses_c_operator(self):
        """Two bool VALUES compared with == / != use the C operators, not a
        z_bool_eq helper (never emitted for the system primitive bool) — so the
        program links and runs."""
        csource = emit_source(
            "eqb: function {a: bool  b: bool} out bool is { return a == b }\n"
            "neb: function {a: bool  b: bool} out bool is { return a != b }\n"
            "main: function is {\n"
            '  if (eqb a: true b: true) then print "eq"\n'
            '  if (neb a: true b: false) then print "neq"\n'
            "}"
        )
        assert "z_bool_eq" not in csource
        output = compile_and_run(csource)
